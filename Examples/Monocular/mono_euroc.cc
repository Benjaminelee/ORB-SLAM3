...
{  
    ...
    // 新增：开启定位模式，仅跟踪，停止localmapping的BA优化，做轻量VO
    ...
    std::cout << "median tracking time: " << vTimesTrack[tot_images/2] << std::endl;
    std::cout << "mean tracking time: " << totaltime/tot_images << std::endl;
                ...
                vector<string> &vstrImages, vector<double> &vTimeStamps)
{
    ...
    while(!fTimes.eof())
    {
        ...
        if(!s.empty())
        {
            ...
            string strTime = s.substr(0, comma_pos);
            string strName = s.substr(comma_pos+1);

            // 时间戳是纳秒，转成秒
            double t = std::stoll(strTime) * 1e-9;
            vTimeStamps.push_back(t);

            vstrImages.push_back(strImagePath + "/" + strName);
